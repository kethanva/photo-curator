...
def score_photos(
    ...
    Base components (sharpness … diversity_bonus) are min-max normalised to
    [0, 1] within the current eligible set, then combined as a weighted sum.

    subject_scores, when provided, add an extra weighted boost on top of the
    base score for photos matching configured subject priorities.  This keeps
    the boost orthogonal to the base ranking so non-matching photos are not
    penalised relative to each other.

    Args:
        records:        list of dicts from the database (all eligible photos)
        weights:        component weights — see config.yaml ranking.weights
        subject_scores: optional {path: boost} from subject_priority.compute_scores()

    Returns:
        {path: score}  — unbounded above 1.0 when subject_boost > 0
    """
    if not records:
        return {}
    ...
    # ── Subject priority boost (additive) ────────────────────────
    # Photos matching configured subject priorities receive a score
    # bonus proportional to their best-matching subject similarity.
    # Photos with no matching subject are unaffected (boost = 0).
    boost_weight = w.get("subject_boost", 0.0)
    if boost_weight > 0.0 and subject_scores:
        subject_boost = np.array(
            [subject_scores.get(p, 0.0) for p in paths], dtype=float
        )
        total = total + boost_weight * subject_boost
